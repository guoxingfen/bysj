...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 物品信息 <span class="c-gray en">&gt;</span> 拾物管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
    <form action="${pageContext.request.contextPath }/admin/reportGoodsManage.action" method="get" id="report_form">
     <div class="text-c"> 举报人数大于或等于：
        <input type="hidden" name="type" value="Get_Goods"/>
	...
	<div class="mt-20">
	 <table class="table table-border table-bordered  table-bg table-sort">
		...
		<tr class="text-c">
				<td><input type="checkbox" value="${adminGoodsInfo.id }" name="get"></td>
				<td>${index.index+1 }</td>
				<td>
...
<!-- 要添加相应的状态功能，没找回，已找回，已删除 -->
				<td class="td-status" id='status'>
				 <c:if test="${adminGoodsInfo.status==0 }">
				   <span class="label label-danger radius">认领中</span>
				 </c:if>
				 <c:if test="${adminGoodsInfo.status==1 }">
				   <span class="label label-success radius">已认领</span>
				 </c:if>
				</td>
				<td>
...
function changeStatus(obj,id){
	layer.confirm('确认该物品已认领吗？',{btn: ['确定', '取消'],title:"提示",icon:3},function(){
		$.ajax({
			type: 'POST',
			...
			success: function(data){
				...
				$(obj).parents("tr").children("td#status").append('<span class="label label-success radius">已认领</span>');
				$(obj).remove();
...
function deleteAllCheck(){
	var ids=new Array();
	$("input:checkbox[name=get]:checked").each(function(key,value){
		ids.push($(this).val());
	})
	var sss="";
